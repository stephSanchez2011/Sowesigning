...
def build_signataires_text(signataires: list) -> str:
    if not signataires:
        return "_Personne n'a encore signé_"
    return "\n".join(f"✅ {s}" for s in signataires)


# ─── Vue bouton "J'ai signé" ──────────────────────────────────────────────────

class SignatureView(discord.ui.View):
    def __init__(self, guild_id: str, today: str, periode: str):
        super().__init__(timeout=None)
        self.guild_id = guild_id
        self.today = today
        self.periode = periode

    @discord.ui.button(label="✅ J'ai signé", style=discord.ButtonStyle.success, custom_id="btn_signe")
    async def signe(self, interaction: discord.Interaction, button: discord.ui.Button):
        ...
        day_data = guild_data.setdefault(self.today, {})
        periode_data = day_data.setdefault(self.periode, {})
        signataires = periode_data.setdefault("signataires", [])

        if pseudo in signataires:
            await interaction.response.send_message("Tu as déjà signé ! 😄", ephemeral=True)
            return

        signataires.append(pseudo)
        save_data(data)

        embed = interaction.message.embeds[0]
        new_fields = [f for f in embed.fields if f.name != "👥 Signataires"]
        embed.clear_fields()
        for f in new_fields:
            embed.add_field(name=f.name, value=f.value, inline=f.inline)
        embed.add_field(
            name="👥 Signataires",
            value=build_signataires_text(signataires),
            inline=False,
        )

        await interaction.message.edit(embed=embed, view=self)
        await interaction.response.send_message(f"Super, **{pseudo}** a signé ! 🎉", ephemeral=True)
...
async def sowesign(
    ...
    embed.add_field(name="👥 Signataires", value="_Personne n'a encore signé_", inline=False)
    embed.set_footer(text=f"Ajouté par {interaction.user.display_name}")

    view = SignatureView(guild_id=guild_id, today=today, periode=periode_finale)
...
async def lien(interaction: discord.Interaction):
    ...
    embed = discord.Embed(title="📎 Derniers liens SoWeSign", color=0x57F287)
    ...
    if not all_days:
        ...
        for day in all_days:
            ...
            for p in ["Matin", "Après-midi"]:
                if p in day_data:
                    info = day_data[p]
                    nb = len(info.get("signataires", []))
                    lines.append(f"**{p}** : [Lien]({info['lien']}) — {nb} signataire(s)")
            if lines:
...
@tree.command(name="purge", description="Supprime les anciens liens (garde les N derniers jours)")
@app_commands.describe(jours="Nombre de jours à conserver (défaut : 7)")
async def purge(interaction: discord.Interaction, jours: int = 7):
    if jours < 1:
        await interaction.response.send_message("❌ Le nombre de jours doit être au moins 1.", ephemeral=True)
        return
    ...
    if not guild_data:
        await interaction.response.send_message("Aucune donnée à purger.", ephemeral=True)
        return

    limite = (datetime.now(PARIS_TZ) - timedelta(days=jours)).strftime("%Y-%m-%d")
    anciens = [day for day in guild_data if day < limite]

    if not anciens:
        await interaction.response.send_message(
            f"✅ Rien à supprimer, tous les liens datent de moins de **{jours} jours**.",
            ephemeral=True,
        )
        return
    ...
    await interaction.response.send_message(
        f"🗑️ **{len(anciens)} jour(s)** supprimé(s) — les **{jours} derniers jours** sont conservés.",
        ephemeral=True,
    )
...
async def aide(interaction: discord.Interaction):
    embed = discord.Embed(title="🤖 Aide — Bot SoWeSign", color=0xFEE75C)
    embed.add_field(
    ...
    embed.add_field(name="/sowesign lien:[url] periode:[Matin|Après-midi]", value="Force la période manuellement.", inline=False)
    embed.add_field(name="/lien", value="Affiche les 5 derniers jours de liens + nombre de signataires.", inline=False)
    embed.add_field(name="/purge [jours]", value="Supprime les entrées de plus de N jours (défaut : 7).", inline=False)
    embed.add_field(name="/aide", value="Affiche ce message d'aide.", inline=False)
    embed.add_field(
        name="⏰ Rappels automatiques",
        value="• **9h00** (Paris) — rappel lien Matin\n• **14h00** (Paris) — rappel lien Après-midi",
        inline=False,
    )
    embed.add_field(
        name="✅ Bouton de signature",
        value="Chaque message contient un bouton pour confirmer sa signature.",
        inline=False,
    )
    await interaction.response.send_message(embed=embed, ephemeral=True)
...
async def send_rappel(periode: str):
    ...
    for guild in bot.guilds:
        ...
        if lien_info:
            signataires = lien_info.get("signataires", [])
            embed = discord.Embed(
            ...
            embed.add_field(name="👥 Signataires", value=build_signataires_text(signataires), inline=False)
            embed.set_footer(text=f"Lien fourni par {lien_info['auteur']}")
            view = SignatureView(guild_id=guild_id, today=today, periode=periode)
            await channel.send(embed=embed, view=view)
        else:
            embed = discord.Embed(
